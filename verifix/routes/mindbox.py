    ...
    with SessionLocal() as db:
        ...
        if file_return and os.path.exists(file_return['file_path']):
            ...
            for order in data.get("orders", []):
                ...
                customer_id = order['customer'].get('ids')
                if customer_id:
                    customer_id = str(customer_id['externalClientId'])
                else:
                    customer_id = ' No Id'
                customer_phone = order['customer'].get('mobilePhone','No phone')
...
def startup_event():
    ...
    trigger = CronTrigger(hour=14, minute=51, second=00,
...
def startup_event_request_report():
    scheduler = BackgroundScheduler()
    trigger = CronTrigger(hour=14, minute=57, second=00,
                          timezone=timezone_tash)  # Set the desired time for the function to run (here, 12:00 PM)
